...
procedure Ejercicio_1 is
    ...
    -- Inicializamos ya el paso previo, con los valores de SC1 y SC2 como constantes
...
begin
    ...
    -- Si no existe el archivo input.txt y no está en la carpeta de src/archivos el programa no funciona
    Comprobar_Archivos;
    ...
    Put_Line("Ejercicio 1: Simulacion de la planta usando valores de SC1 y SC2 constantes");
    New_Line;

    while not Fin loop
        Leer_Entrada(Iteracion, SR1, ST4, ST3, Fin);

        -- Si Fin = True, se ha terminado el archivo
        if Fin then
            exit;
        end if;
        ...
        -- Se inicializan porque se calculan en la función de Calcular_Paso
        Paso_Actual.ST1 := 0.0;
        Paso_Actual.ST2 := 0.0;
        Paso_Actual.SD1 := 0.0;
